...
public class AddDocumentPage extends BasePage {
	...
	By editTxtDocumentName = By.xpath("//input[contains(@name,'EditFormControl$txtDocName')]");
	public void editDocumentName (String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(editTxtDocumentName);
		util.inputTextAndPressTab(editTxtDocumentName, value);
	}
	By editDocumentCategory = By.xpath("//input[contains(@name,'EditFormControl$ddlDocCat$radYALDropDownList' )]");
	public void editDocumentCategory(String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(editDocumentCategory);
		util.selectValueFromDropdown(editTxtDocumentName, value);
	}
	By drpDraftFinal = By.xpath("//input[contains(@name,'ddlDocDraftFinal$radYALDropDownList' )]");
	public void selectDraftFinal(String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(drpDraftFinal);
		util.selectValueFromDropdown(drpDraftFinal, value);
	}
	
	By txtExecutionDate = By.xpath("//input[contains(@name,'EditFormControl$dtExecDate$dateInput' )]");
	public void addExecutionDate (String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(txtExecutionDate);
		util.inputTextAndPressTab(txtExecutionDate, value);
	}
	
	By txtDocumentDescription = By.xpath("//textarea[contains(@name,'EditFormControl$txtDocDesc')]");
	public void addDocumentDescription (String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(txtDocumentDescription);
		util.inputTextAndPressTab(txtDocumentDescription, value);
	}
	By txtDocumentTitle = By.xpath("//input[contains(@name,'EditFormControl$txtTitle')]");
	public void addDocumentTitle(String value) {
		if (!commonFunction.checkNA(value))
			util.waitUntilElementDisplay(txtDocumentTitle);
		util.inputTextAndPressTab(txtDocumentTitle, value);
	}
	By btnUpdate = By.xpath("//input[contains(@name,'btnUpdate')]");
	By txtSuccessMessage = By.xpath("//*[text()='Changes saved successfully!']");
	...
	public void editDocument(Map<String, String> map, String testcaseName) {
		...
		try {
			...
			log("STEP 4:  User can Enter Document Name :", Status.PASS);
		} catch (Exception e) {
			log("STEP 4:  User can not Enter Document Name :", Status.FAIL);
			throw new RuntimeException("STEP 4:  User can not Enter Document Name :");
		}
		try {
			editDocumentCategory(map.get(Excel.EditDocumentCategory));
			log("STEP 5:  User can Enter Document Category :", Status.PASS);
		} catch (Exception e) {
			log("STEP 5:  User can not Enter Document Category :", Status.FAIL);
			throw new RuntimeException("STEP 5:  User can not Enter Document Category :");
		}
		try {
			selectDraftFinal(map.get(Excel.Draft));
			log("STEP 6:  User can Enter Draft :", Status.PASS);
		} catch (Exception e) {
			log("STEP 6:  User can not Enter Draft :", Status.FAIL);
			throw new RuntimeException("STEP 6:  User can not Enter Draft :");
		}
		
		try {
			addExecutionDate(map.get(Excel.ExecutionDate));
			log("STEP 7:  User can Enter ExecutionDate :", Status.PASS);
		} catch (Exception e) {
			log("STEP 7:  User can not Enter ExecutionDate :", Status.FAIL);
			throw new RuntimeException("STEP 7:  User can not Enter ExecutionDate :");
		}
		
		try {
			addDocumentDescription(map.get(Excel.EditDocumentDescription));
			log("STEP 8:  User can Enter ExecutionDate :", Status.PASS);
		} catch (Exception e) {
			log("STEP 8:  User can not Enter ExecutionDate :", Status.FAIL);
			throw new RuntimeException("STEP 8:  User can not Enter ExecutionDate :");
		}
		try {
			addDocumentTitle(map.get(Excel.DocumentTitle));
			log("STEP 9:  User can Enter DocumentTitle :", Status.PASS);
		} catch (Exception e) {
			log("STEP 9:  User can not Enter DocumentTitle :", Status.FAIL);
			throw new RuntimeException("STEP 9:  User can not Enter DocumentTitle :");
		}
		try {
			util.click(btnUpdate);
			util.waitUntilElementDisplay(txtSuccessMessage);
			log("STEP 10:  Document edit Successfully :", Status.PASS);
		} catch (Exception e) {
			log("STEP 10:   Document not edit Successfully :", Status.FAIL);
			throw new RuntimeException("STEP 10:  Document not edit Successfully :");
		}
		}
		else {
			System.out.println("Edit Funcationality Not Application for ROW and ALT");
		}
	}
